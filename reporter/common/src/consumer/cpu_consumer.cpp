...
CpuConsumer::CpuConsumer(int cpu_count) : Consumer(std::make_unique<CpuReporter>(cpu_count)) {
...
void CpuConsumer::consume() {
  ...
  old_stats.cpu_core_usage_ = std::shared_ptr<CpuUsageStats::usage[]>(new CpuUsageStats::usage[(int)usage.cpu_core_count_]);
  ...
  while(is_consuming_) {
    ...
    cpu_usage.cpu_usage_data.usage_ = 100.0f * (usage.master_cpu_usage_.used_ - old_stats.master_cpu_usage_.used_)/(usage.master_cpu_usage_.total_ - old_stats.master_cpu_usage_.total_);
    ...
    for(std::uint8_t i = 0; i < usage.cpu_core_count_; i++) {
     cpu_usage.type  = common::RequestTypes::kCpuUsage;
     cpu_usage.cpu_usage_data = {static_cast<std::uint8_t>(i + 1), 100.0f * (usage.cpu_core_usage_[i].used_ - old_stats.cpu_core_usage_[i].used_)/(usage.cpu_core_usage_[i].total_ - old_stats.cpu_core_usage_[i].total_)};
     req.data.emplace_back(std::move(cpu_usage));
    }
