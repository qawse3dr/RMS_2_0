...
// this is not be the case and it will be changed to work with any number of
// adapter but as This is a PoC I am just testing that the technology will work
...
void getNetworkUsage(struct network_usage usage[INTERFACE_COUNT],
  ...
  for (int i = 0; EOF != fscanf(fp,
                                " %s %ld %*d %*d %*d %*d %*d %*d %*d %ld %*d "
                                "%*d %*d %*d %*d %*d %*d\n",
                                interface_names[i], &(usage[i].receive),
                                &(usage[i].transmit));
       i++) {
  ...
  while (1) {
    ...
    for (int i = 0; i < INTERFACE_COUNT; i++) {
      printf("%s Download: %ld kB/s\n", interface_names[i],
             (usage[i].receive - old_usage[i].receive) / 1024);
      printf("%s   Upload: %ld kB/s\n", interface_names[i],
             (usage[i].transmit - old_usage[i].transmit) / 1024);
      old_usage[i] = usage[i];
    }
