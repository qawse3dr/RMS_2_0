...
void getCpuUsage(struct cpu_usage* cpu_usage,
  ...
  while (fscanf(fp, "cpu%d %ld %ld %ld %ld %ld %ld %ld %*ld %*ld %*ld\n",
                &cpu_num, &user, &nice, &system, &idle, &iowait, &irq,
                &softirq)) {
    cpu_core_usage[cpu_num].total =
...
int main() {
  ...
  while (1) {
    ...
    printf("master cpu usage: %.2f%%\n",
           100.0 * (master_cpu_usage.used - master_cpu_usage_old.used) /
               (master_cpu_usage.total - master_cpu_usage_old.total));

    for (int i = 0; i < CPU_CORES; i++) {
      printf("cpu%d usage: %.2f%%\n", i,
             100.0 * (cpu_core_usage[i].used - cpu_core_usage_old[i].used) /
                 (cpu_core_usage[i].total - cpu_core_usage_old[i].total));
